...
class ProfilePlotter:
    ...
    def __init__(self, web_view):
        ...
        self.min_y = 0
        self.max_y = 10
        # Offsets pour déplacement des étiquettes
        self.offset_FE_x = 0.0
        self.offset_FE_y = 0.0
        self.offset_TN_x = 0.0
        self.offset_TN_y = 0.0
    ...
    def _create_plotly_figure(self, profile_data, settings):
        ...
        for cana in profile_data['canalisations']:
            ...
            if (cana.get('z_amont') is not None and cana.get('z_aval') is not None and
                ...
                # Tracer la génératrice supérieure uniquement si les valeurs sont valides
                dn = cana.get('dn', 0.2)  # Diamètre par défaut: 200mm

                if (isinstance(dn, (int, float)) and 
                    cana.get('z_amont') is not None and 
                    cana.get('z_aval') is not None and 
                    not math.isnan(cana['z_amont']) and 
                    not math.isnan(cana['z_aval'])):
                
                    fig.add_trace(go.Scatter(
                        x=[cana['start_distance'], cana['end_distance']],
                        y=[cana['z_amont'] + dn, cana['z_aval'] + dn],
                        mode='lines',
                        name='Génératrice Supérieure',
                        line=dict(color='lightblue', width=2, dash='dash'),
                        hovertemplate='Cana ' + f"{cana.get('id', '')}" + '<br>Distance: %{x:.2f}m<br>GS: %{y:.2f}m<extra></extra>',
                        showlegend=not first_gs_added
                    ))
                    first_gs_added = True
                
                # Ajouter le texte de pente si disponible
                slope_sign = 2   # alternance haut / bas pour les pentes
                if cana.get('pente') is not None and settings.get('show_pente', True):
                    ...
                    # décalage verticale 
                    slope_sign *= -1
                    slope_offset = slope_sign * (self.max_y - self.min_y) * 0.025
                    
                    fig.add_annotation(
                        x=mid_x,
                        y = mid_y + slope_offset, #+ 0.2,  # Décalage au-dessus de la canalisation
        ...
        offset_sign = 2   # alterne gauche / droite
        for regard in profile_data['regards']:
            if (regard.get('distance') is not None and 
                ...
                offset_sign *= -1
                y_offset = offset_sign * (self.max_y - self.min_y) * 0.03   # 3% de la hauteur du graphique
                ...
                # Ajout des étiquettes FE et TN si demandé avec décalage intelligent 
                if settings.get('show_fe', True):
                    fig.add_annotation(
                        x=x + self.offset_FE_x, # + x_offset, #x=x + dn/2 + 0.6, # regalge du decalage de l"étiquettes
                        y=y_fe + y_offset + self.offset_FE_y, #(self.max_y - self.min_y) * 0.02,  # Décalage vertical léger (2%)
                        text=f"<b>FE: {y_fe:.2f}</b>",
                        showarrow=False,
                ...
                if settings.get('show_tn', True):
                    fig.add_annotation(
                        x=x + self.offset_TN_x, # + x_offset, #x=x + dn/2 + 0.6,
                        y=y_tn + y_offset + self.offset_TN_y, #(self.max_y - self.min_y) * 0.05,  # Plus haut que FE (5%)
                        text=f"<b>TN: {y_tn:.2f}</b>",
                        showarrow=False,
    ...
    def capture_to_file(self, filename=None):
        """Capture le contenu visible du QWebEngineView et l'enregistre dans le dossier d'export."""
