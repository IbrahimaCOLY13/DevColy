...
class LiaisonminiCanaTab(QtWidgets.QDialog, FORM_CLASS):
    ...
    def run_model(self):
        ...
        try:
            ...
            params = {
                'couche_de_cana': layer_cana,
                'couche_de_regard': layer_regard,
                'identifiant_unique_cana': identifiant_cana,
                'identifiant_unique_regard': identifiant_regard,
                'Output_attention_croisement': self.line_attention.text() or 'TEMPORARY_OUTPUT',
                'Couche_regard_complte_avec_croisement': self.line_croisement.text() or 'TEMPORARY_OUTPUT',
                'Relation_canaregard_sans_croisement': self.line_relation.text() or 'TEMPORARY_OUTPUT',
                'CanaEnDouble': self.line_double.text() or 'TEMPORARY_OUTPUT'
            }
            ...
            # Charger les couches de sortie si demandé
            if self.check_open_attention.isChecked() and 'Output_attention_croisement' in result:
                output = result['Output_attention_croisement']
                if isinstance(output, QgsVectorLayer):
                    output.setName('Attention Croisement')
                    QgsProject.instance().addMapLayer(output)
                elif isinstance(output, str):
                    self.iface.addVectorLayer(output, 'Attention Croisement', 'ogr')
            
            if self.check_open_croisement.isChecked() and 'Couche_regard_complte_avec_croisement' in result:
                output = result['Couche_regard_complte_avec_croisement']
                if isinstance(output, QgsVectorLayer):
                    output.setName('Regard avec Croisement')
                    QgsProject.instance().addMapLayer(output)
                elif isinstance(output, str):
                    self.iface.addVectorLayer(output, 'Regard avec Croisement', 'ogr')
            
            if self.check_open_relation.isChecked() and 'Relation_canaregard_sans_croisement' in result:
                output = result['Relation_canaregard_sans_croisement']
                if isinstance(output, QgsVectorLayer):
                    output.setName('Relation Cana-Regard')
                    QgsProject.instance().addMapLayer(output)
                elif isinstance(output, str):
                    self.iface.addVectorLayer(output, 'Relation Cana-Regard', 'ogr')
            
            if self.check_open_double.isChecked() and 'CanaEnDouble' in result:
                output = result['CanaEnDouble']
                if isinstance(output, QgsVectorLayer):
                    output.setName('Cana en Double')
                    QgsProject.instance().addMapLayer(output)
                elif isinstance(output, str):
                    self.iface.addVectorLayer(output, 'Cana en Double', 'ogr')
