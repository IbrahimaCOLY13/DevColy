...
class LiaisonMinicana(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        self.addParameter(QgsProcessingParameterVectorLayer('couche_de_cana', 'couche de cana', types=[QgsProcessing.TypeVectorLine], defaultValue=None, optional=False))
        self.addParameter(QgsProcessingParameterVectorLayer('couche_de_regard', 'couche de regard', types=[QgsProcessing.TypeVectorPoint], defaultValue=None, optional=False))
    ...
    def processAlgorithm(self, parameters, context, model_feedback):
        ...
        alg_params = {
            ...
            'INPUT': self.parameterAsVectorLayer(parameters, 'couche_de_regard', context),
        ...
        alg_params = {
            ...
            'INPUT': self.parameterAsVectorLayer(parameters, 'couche_de_cana', context),
        ...
        # Lste des canas qui intercsete le point central - via jointure spatiale
        alg_params = {
            'DISCARD_NONMATCHING': False,
            'INPUT': outputs['TamponPointCentrale']['OUTPUT'],
            'JOIN': outputs['RenommerLeChampIdCana']['OUTPUT'],
            'JOIN_FIELDS': ['idUnique_cana'],
            'METHOD': 0,  # Créer des entités distinctes pour chaque entité localisée (un-à-plusieurs)
            'PREDICATE': [0],  # intersecte
            'PREFIX': '',
            'OUTPUT': QgsProcessing.TEMPORARY_OUTPUT
        }
        outputs['JointureSpatialeCana'] = processing.run('native:joinattributesbylocation', alg_params, context=context, feedback=feedback, is_child_algorithm=True)
        
        # Aggrégation pour créer la liste
        alg_params = {
            'FIELD_LENGTH': 250,
            'FIELD_NAME': 'Liste_cana',
            'FIELD_PRECISION': 0,
            'FIELD_TYPE': 2,  # Texte
            'FORMULA': '"idUnique_cana"',
            'INPUT': outputs['JointureSpatialeCana']['OUTPUT'],
            'OUTPUT': QgsProcessing.TEMPORARY_OUTPUT
        }
        ...
        alg_params = {
            'EXPRESSION': 'array_length(string_to_array("Liste_cana", \';\')) > 2',
            'INPUT': outputs['LsteDesCanasQuiIntercseteLePointCentral']['OUTPUT'],
            'OUTPUT': QgsProcessing.TEMPORARY_OUTPUT
        }
        outputs['FiltreSurLesListePourRcuprerCeuxPlusDe2'] = processing.run('native:extractbyexpression', alg_params, context=context, feedback=feedback, is_child_algorithm=True)
        ...
        alg_params = {
            'FIELDS': ['Liste_cana'],
            'INPUT': outputs['FiltreSurLesListePourRcuprerCeuxPlusDe2']['OUTPUT'],
        ...
        # Calcul champs Liste_paire - via jointure spatiale  
        alg_params = {
            'DISCARD_NONMATCHING': False,
        ...
        outputs['JointureSpatialePaire'] = processing.run('native:joinattributesbylocation', alg_params, context=context, feedback=feedback, is_child_algorithm=True)
        
        # Aggrégation des idUnique_cana par Rang
        alg_params = {
            'AGGREGATES': [{'name': 'Liste_paire', 'aggregate': 'concatenate', 'delimiter': ';', 'input': '"idUnique_cana"', 'length': 80, 'precision': 0, 'sub_type': 0, 'type': 10, 'type_name': 'text'}],
            'GROUP_BY': '"Rang"',
            'INPUT': outputs['JointureSpatialePaire']['OUTPUT'],
            'OUTPUT': QgsProcessing.TEMPORARY_OUTPUT
        }
        outputs['CalculChampsListe_paire'] = processing.run('native:aggregate', alg_params, context=context, feedback=feedback, is_child_algorithm=True)
        ...
        # Filtre d'entité - Extraction des croisements (Croisement = 'oui')
        alg_params = {
            'EXPRESSION': '"Croisement" = \'oui\'',
            'INPUT': outputs['CalculChampsCroisement']['OUTPUT'],
            'OUTPUT': parameters['Output_attention_croisement']
        }
        outputs['FiltreDentit_Attention_croisement'] = processing.run('native:extractbyexpression', alg_params, context=context, feedback=feedback, is_child_algorithm=True)
        results['Output_attention_croisement'] = outputs['FiltreDentit_Attention_croisement']['OUTPUT']
        ...
        alg_params = {
            'FIELDS': ['Liste_paire'],
            'INPUT': outputs['FiltreDentit_Attention_croisement']['OUTPUT'],
            'OUTPUT': QgsProcessing.TEMPORARY_OUTPUT
        }
        outputs['SupprimerLesDoublonsParAttribut'] = processing.run('native:removeduplicatesbyattribute', alg_params, context=context, feedback=feedback, is_child_algorithm=True)
        ...
        alg_params = {
            'GRID_SIZE': None,
            'INPUT': outputs['CalculChampsCroisement']['OUTPUT'],
            'OVERLAY': outputs['FiltreDentit_Attention_croisement']['OUTPUT'],
            'OUTPUT': QgsProcessing.TEMPORARY_OUTPUT
        }
