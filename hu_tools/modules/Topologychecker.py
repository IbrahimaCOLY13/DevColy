...
class TopologyChecker:
    ...
    def run_all_checks(self):
        ...
        # Vérification de la cohérence des altitudes
        all_errors.extend(self.check_coherence_altitudes())
    ...
    def check_connections_branches_network(self):
        ...
        for branch_type, allowed_connections in connection_rules.items():
            ...
            for branch_feature in branch_layer.getFeatures():
                ...
                # Obtenir le dernier point du branchement (point d'arrivée)
                # Récupérer le dernier point compatible LineString ou MultiLineString
                end_point = extraire_dernier_point_geom(branch_geom)
                if end_point is None:
                    continue  # ignorer les géométries non conformes
    ...
    def check_coherence_altitudes(self):
        """
        Vérifie la cohérence des altitudes entre :
        - Canalisations (Altitude_Amont = amont, Altitude_Aval = aval)
        - Points (Altitude_Point = altitude des regards/boîtes/exutoires)
        """
        erreurs = []

        # Champs utilisés
        CHAMP_ALTITUDE_POINT = 'COORLAMZ'
        CHAMP_ALTITUDE_AMONT = 'ALTDEB'
        CHAMP_ALTITUDE_AVAL = 'ALTFIN'
        tolerance = 0.20  # marge d'erreur en mètres

        # Parcourir toutes les couches
        for clef_couche, couche_ligne in self.layers.items():
            if couche_ligne and couche_ligne.geometryType() == QgsWkbTypes.LineGeometry:

                # Identifier la couche de points correspondante
                clef_points = clef_couche.replace("CANA", "POINT")  # À adapter 
                ...
                for canalisation in couche_ligne.getFeatures():
                    ...
                    # Récupérer les altitudes amont et aval de la canalisation
                    altitude_amont = canalisation[CHAMP_ALTITUDE_AMONT]
                    altitude_aval = canalisation[CHAMP_ALTITUDE_AVAL]

                    # Vérifier présence des altitudes
                    if altitude_amont is None or altitude_aval is None:
                        self.error_id += 1
                        erreurs.append({
                            'id': self.error_id,
                            'type': 'Altitude manquante',
                            'description': f"Canalisation ID {id_canalisation} sans altitude amont ou aval.",
                            'layer': couche_ligne.name(),
                            'feature_id': id_canalisation
                        })
                        continue

                    # Points de début et fin
                    if geom_canal.isMultipart():
                        polyline = geom_canal.asMultiPolyline()[0]  # On prend la première sous-ligne
                    else:
                        polyline = geom_canal.asPolyline()

                    # Sécurisation si la géométrie est vide
                    if not polyline:
                        continue
                    ...
                    # Vérification cohérence altitude amont
                    if point_amont:
                        altitude_point_amont = point_amont[CHAMP_ALTITUDE_POINT]
                        if abs(altitude_point_amont - altitude_amont) > tolerance:
                            self.error_id += 1
                            erreurs.append({
                                'id': self.error_id,
                                'type': 'Incohérence altitude amont',
                                'description': f"Canalisation {id_canalisation} : ALDEP={altitude_amont} différent de COORLAMZ={altitude_point_amont}",
                                'layer': couche_ligne.name(),
                                'feature_id': id_canalisation
                            })

                    # Vérification cohérence altitude aval
                    if point_aval:
                        altitude_point_aval = point_aval[CHAMP_ALTITUDE_POINT]
                        if abs(altitude_point_aval - altitude_aval) > tolerance:
                            self.error_id += 1
                            erreurs.append({
                                'id': self.error_id,
                                'type': 'Incohérence altitude aval',
                                'description': f"Canalisation {id_canalisation} : ALTPIQ={altitude_aval} différent de COORLAMZ={altitude_point_aval}",
                                'layer': couche_ligne.name(),
                                'feature_id': id_canalisation
                            })
                    ...
                    if altitude_amont <= altitude_aval:
                        ...
                        erreurs.append({
                            'id': self.error_id,
                            'type': 'Sens d’écoulement incorrect',
                            'description': f"Canalisation {id_canalisation} : ALDEP {altitude_amont} <= ALTPIQ {altitude_aval}",
                            'layer': couche_ligne.name(),
                            'feature_id': id_canalisation
                        })
